...
function loadLocalData() {
  fetch("http://localhost/finalProject/dane/maslo.txt")
    .then((response) => {
  ...
  fetch("http://localhost/finalProject/dane/pieczywo.txt")
    .then((response) => {
      return response.text();
    })
    .then((dane) => {
  ...
  fetch("http://localhost/finalProject/dane/kawa.txt")
    .then((response) => {
      return response.text();
    })
    .then((dane) => {
  ...
  fetch("http://localhost/finalProject/dane/products-01.jpg")
  ...
  fetch("http://localhost/finalProject/dane/products-02.jpg")
    .then((response) => response.blob())
    .then(function (myBlob) {
      var objectURL = URL.createObjectURL(myBlob);
  ...
  fetch("http://localhost/finalProject/dane/products-03.jpg")
    .then((response) => response.blob())
    .then(function (myBlob) {
      var objectURL = URL.createObjectURL(myBlob);
...
function resetButton() {
  if (
    document.querySelector(".skladanieZamowienia").classList.contains("temp")
  ) {
...
function sprawdziZloz() {
  ...
  if (localStorage.getItem(nazwaMasla.value)) {
    if (
      edytowanaNazwa != JSON.parse(localStorage.getItem(nazwaMasla.value)).nazwa
    ) {
      poprawnie = false;
      document.getElementById("nazwa_error").innerHTML =
